...
export default function CheckMyBalanceTokenAAndB(props) {
  ...
  const checkIsMyInputHigherThanMyBalance = async (addressTokenA, addressTokenB) => {
    try {
      ...
      const receiptTokenB = await web3.tolar.tryCallTransaction(
        connectedAccount,
      ...
      const { 0: myBalanceTokenB } = web3.eth.abi.decodeParameters(
        ["uint256"],
        receiptTokenB.output
      );

      const myBalanceTokenBToNum = new BigNumber(myBalanceTokenB).shiftedBy(-18).toFixed(0);

      const amountIntTokenBToNum = parseFloat(values.amountOut)


      const receiptTokenA = await web3.tolar.tryCallTransaction(
        connectedAccount,
      ...
      const { 0: myBalanceTokenA } = web3.eth.abi.decodeParameters(
        ["uint256"],
        receiptTokenA.output
      );

      const myBalanceToNum = new BigNumber(myBalanceTokenA).shiftedBy(-18).toFixed(0);

      const amountIntToNum = parseFloat(values.amountIn)

      if (amountIntToNum > myBalanceToNum || amountIntTokenBToNum>myBalanceTokenBToNum ) {
